...
void quick_sort(typename T::iterator first, typename T::iterator last, std::stack<std::pair<typename T::iterator, typename T::iterator>>& tasks) { // 5 3 2 4 5 7 8 3 6 1 1 6(o)
	...
	if (last - first < 10000) {
		quick_sort<std::vector<int>>(first, first + medium, tasks);
		quick_sort<std::vector<int>>(first + right_medium + 1, last, tasks);
		return;
	}
...
template<class T>
void my_sort(typename T::iterator first, typename T::iterator last, size_t num_threads = NUM_THREADS) {
